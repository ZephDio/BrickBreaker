...
function drawBall(ballState, state) {
  if (state.isRedBuffActive) {
    ctx.drawImage(
      billeRed,
      ballState.x - ballState.radius / 2,
      ballState.y -
        (ballState.radius / 2, ballState.radius * 2, ballState.radius * 2)
    );
  } else {
    ctx.drawImage(
      bille,
      ballState.x - ballState.radius / 2,
      ballState.y - ballState.radius / 2,
      ballState.radius * 2,
      ballState.radius * 2
    );
  }
...
function renderGameOverScreen(state) {
  ...
  ctx.drawImage(
    GameOver,
    (state.width - GameOver.width) / 2,
    (state.height - GameOver.height) / 2
  );
}
