...
class Game {
  ...
  updateBuffPods() {
    for (let i = 0; i < this.buffPods.length; i++) {
      ...
      if (
        this.plate.contains(
          this.buffPods[i].x + this.buffPods[i].width / 2,
          this.buffPods[i].y + this.buffPods[i].height / 2
        )
      ) {
        switch (this.buffPods[i].effect) {
          case "redBuff":
            this.buffManager.applyRedBuff(10);
            break;
          case "blueBuff":
            this.buffManager.applyBlueBuff(12);
            break;
          case "multiBall":
            let ball = new Ball(
              true,
      ...
      } else if (this.buffPods[i].y > this.height) {
        this.buffPods.splice(i, 1);
      }
    }
  }
    ...
    for (let i = 0; i < this.balls.length; i++) {
      ...
      for (const brick of this.bricks) {
        if (brick.contains(ball)) {
          ...
          ball.alpha = ball.alpha % (Math.PI * 2);
          const conditionBouncingLeft =
            (ball.alpha < Math.PI / 2 || ball.alpha > Math.PI * (3 / 2)) &&
            ball.x < brick.x + brick.width / 2;
          const conditionBouncingRight =
            ball.alpha > Math.PI / 2 &&
            ball.alpha < Math.PI * (3 / 2) &&
            ball.x > brick.x + brick.width / 2;
          const conditionBouncingTop =
            ball.alpha < Math.PI && ball.y < brick.y + brick.height / 2;
          const conditionBouncingBottom =
            ball.alpha > Math.PI && ball.y > brick.y + brick.height / 2;

          if (
            brick.bouncingOnSide(ball) &&
            !this.buffManager.isRedBuffActive() &&
          ...
          if (
            brick.bouncingOnHorizontalSide(ball) &&
            !this.buffManager.isRedBuffActive() &&
  ...
  updatePlate() {
    if (
      this.buffManager.isBlueBuffActive() &&
